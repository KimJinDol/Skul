...
void CRenderManager::SetRenderTypeTint(Render_Type_2D RenderType, unsigned int r, unsigned int g, unsigned int b, unsigned int a)
{
	m_RenderTypeTint[RenderType] = Vector4(r, g, b, a);
}

void CRenderManager::SetRenderTypeTint(Render_Type_2D RenderType, Vector4 Color)
{
	m_RenderTypeTint[RenderType] = Color;
}
...
void CRenderManager::Render(float DeltaTime)
{
	/*
	렌더 순서
	1. DiffuseTarget에 Distortion적용을 하지 않을 오브젝트들을 그려낸다.
	2. RenterTarget을 Distortion으로 바꾼 후 Distortion을 적용할 옵젝을 DiffuseTarget의 텍스처 픽셀정보와 혼합하여 그려낸다.
	*/
	...
	// m_AlphaBlend->SetState(); 여기 활성화해야 에디터 렌더링 됨
	...
	Offset = 0;
	// NullBuffer는 트라이앵글 스트립으로 그려야 함.
	CONTEXT->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
	// NullBuffer이기 때문에 인덱스나, 정점 데이터, 입력 레이아웃은 모두 nullptr이다.
	CONTEXT->IASetVertexBuffers(0, 0, nullptr, nullptr, &Offset);
	CONTEXT->IASetIndexBuffer(nullptr, DXGI_FORMAT_UNKNOWN, 0);
...
void CRenderManager::Render2D(float DeltaTime)
{
	...
	for (int k = 0; k < m_RenderList2DCount[RT2D_Background]; ++k)
	{
		if (!m_pRenderList2D[RT2D_Background][k]->IsStart())
			m_pRenderList2D[RT2D_Background][k]->Start();

		if (m_pRenderList2D[RT2D_Background][k]->GetMaterial(0))
			m_pRenderList2D[RT2D_Background][k]->GetMaterial(0)->SetTintColor(m_RenderTypeTint[RT2D_Background]);
		m_pRenderList2D[RT2D_Background][k]->Render(DeltaTime);
	}
	...
	{
		if (!m_pRenderList2D[RT2D_MAP][j]->IsStart())
			m_pRenderList2D[RT2D_MAP][j]->Start();

		if (m_pRenderList2D[RT2D_MAP][j]->GetMaterial(0))
			m_pRenderList2D[RT2D_MAP][j]->GetMaterial(0)->SetTintColor(m_RenderTypeTint[RT2D_MAP]);
	...
	for (int i = RT2D_Back; i < RT2D_End; ++i)
	{
		...
		//	CRenderManager::Sort2DObject); 스컬에선 소트가 필요 없지?

		for (int j = 0; j < m_RenderList2DCount[i]; ++j)
		{
			if (!m_pRenderList2D[i][j]->IsStart())
				m_pRenderList2D[i][j]->Start();

		/*	if (m_pRenderList2D[i][j]->GetMaterial(0))
				m_pRenderList2D[i][j]->GetMaterial(0)->SetTintColor(m_RenderTypeTint[i]);*/
...
void CRenderManager::AddPrimitiveComponent(CPrimitiveComponent* pPrimitive)
{
	if (pPrimitive->GetPrimitiveType() == PrimitiveComponent_Type::Primitive2D)
	{
		// 화면에 그려져야 하는지 판단한다.
		CScene* Scene = CSceneManager::GetInst()->GetScene();
		// Map은 컴포넌트 단위가 아니라 타일별로 컬링 작업을 따로 진행할 것이기 때문에 여기에선 스킵.
