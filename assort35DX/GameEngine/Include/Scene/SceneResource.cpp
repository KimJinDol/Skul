...
bool CSceneResource::Init()
{
	...
	pMesh = CResourceManager::GetInst()->FindMesh("ColliderLine");

	if (pMesh)
	{
		pMesh->m_pScene = m_pScene;
		pMesh->AddRef();
		m_mapMesh.insert(std::make_pair("ColliderLine", pMesh));
	}
...
bool CSceneResource::CreateMaterial(const std::string& Name, const std::string& TextureName, const TCHAR* FileName, const std::string& PathName)
{
	CMaterial* pMaterial = FindMaterial(Name);
...
bool CSceneResource::CreateAnimationSequence2D(const std::string& SequenceName, float PlayTime, const std::string& TextureName, unsigned int x, unsigned int y, unsigned int Count, const Vector2& FrameSize)
{
	CAnimationSequence2D* pSequence = FindAnimationSequence2D(SequenceName);
	...
	for (int i = 0; i < Count; ++i)
	{
		...
		pSequence->SetFrameSize(FrameSize);
	...
	const Vector2& Start, const Vector2& End)
{
	...
	pSequence->SetFrameSize(End - Start);
...
bool CSceneResource::AddAnimationSequence2DFrame(const std::string& SequenceName, unsigned int x, unsigned int y, unsigned int Count, const Vector2& FrameSize)
{
	CAnimationSequence2D* pSequence = FindAnimationSequence2D(SequenceName);
...
CAnimationSequence2D* CSceneResource::FindAnimationSequence2D(const std::string& Name)
{
	...
	{
		CAnimationSequence2D* pSequence = CResourceManager::GetInst()->FindAnimationSequence2D(Name);

		if (pSequence)
		{
			m_mapAnimationSequence2D.insert(std::make_pair(Name, pSequence));
			return pSequence;
		}
		else
			return nullptr;
