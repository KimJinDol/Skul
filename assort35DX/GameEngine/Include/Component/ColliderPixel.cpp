...
void CColliderPixel::SetTextureFullPath(const TCHAR* FullPath)
{
	...
	// BMP확장자 이미지는 상하 반전으로 나오기 때문에 따로 후처리가 필요함
	if (strcmp(strExt, ".BMP") == 0)
	{
		// 한 줄 씩 처리해준다.
		size_t	CopySize = m_Info.Width * 4;	// 32비트(4바이트)기준으로 처리할 것이기 때문에 * 4를 해줌

		unsigned char* Line = new unsigned char[CopySize + 1];	// CopySize에 +1을 해주는 이유는 공백으로 사용할 것이기 땜

		size_t	HalfHeight = m_Info.Height / 2;

		// 위에서 i번째 줄과 아래에서 i번째 줄의 픽셀 정보를 서로 스왑해준다.
