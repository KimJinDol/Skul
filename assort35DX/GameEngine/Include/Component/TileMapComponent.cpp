...
{
	m_Mesh = (CSpriteMesh*)m_pScene->GetResource()->FindMesh(Name);	// 리소스 매니저에 만들어뒀던 메시를 가져온 후

	Vector3 Min = m_Mesh->GetMin();	// 메시의 최소 정점과 최대 정점 위치를 구한다.
	Vector3 Max = m_Mesh->GetMax();

	m_pTransform->SetMeshSize(Max - Min);	// Min, Max를 이용하여 메시의 사이즈를 구해준다.

	m_vecMaterialSlot.clear();

	const std::vector<CSharedPtr<CMaterial>>* pMaterialSlots =	// 메시의 머터리얼들을 구해온다.
		m_Mesh->GetMaterialSlots();
	...
	for (; iter != iterEnd; ++iter)
	{
		...
		pClone->SetScene(m_pScene);	// Scene을 새로 설정해준다.
		m_vecMaterialSlot.push_back(pClone);
...
void CTileMapComponent::SetFrameMax(int x, int y)
{
	m_FrameMaxX = x;
	m_FrameMaxY = y;
	// 타일 이미지 텍스처의 타일 가로 세로의 최대 개수를 알았으니, 이를 이용해 타일 한 칸의 크기도 구해준다.
...
int CTileMapComponent::GetTileIndexX(const Vector3& Pos)
{
	...
	{
		float	ConvertX = Pos.x - GetWorldPos().x; // 타일의 인덱스를 구하기 위해 원점 (0, 0)으로 기준점을 바꾼다.
...
bool CTileMapComponent::Init()
{
	...
	// Navigation->DeleteTileMapData(this); 11.01 터져서 일단 막아놓음
...
void CTileMapComponent::Render(float DeltaTime)
{
	...
	Vector3	LB = Camera->GetWorldPos(); // 카메라 위치는 화면의 Left Bottom을 기준으로 한다.
	...
	/*--------------------------위는 PostUpdate에 있던 내용---------------------------*/


	m_WorldBuffer->SetShader(30, CBT_VERTEX); // 30번 구조화 버퍼 레지스터의 쉐이더 정보를 등록해준다.

	m_CBuffer->UpdateCBuffer(); // TileMapCBuffer 의 정보를 새로 업데이트 하여 쉐이더로 보내준다.
...
void CTileMapComponent::LoadFullPath(const TCHAR* FullPath)
{
	...
	m_vecMaterialSlot[0]->Release(); // ReasourceManager에 포함이 안되기 때문에 Release 해주는 것이라고.. 성우좌가 그랬음
