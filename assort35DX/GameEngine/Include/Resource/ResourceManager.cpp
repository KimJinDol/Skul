...
bool CResourceManager::Init()
{
	...
	if (!CreateMesh(Mesh_Type::Sprite, "ColliderLine", LinePos, 2,
		sizeof(Vector3), D3D11_USAGE_IMMUTABLE, D3D11_PRIMITIVE_TOPOLOGY_LINELIST))
		return false;
	...
	CreateSampler("Background", D3D11_FILTER_MIN_MAG_POINT_MIP_LINEAR, 2, D3D11_TEXTURE_ADDRESS_WRAP, D3D11_TEXTURE_ADDRESS_CLAMP);
	...
	pSampler = FindSampler("Background");
	CONTEXT->VSSetSamplers(3, 1, &pSampler);
	CONTEXT->PSSetSamplers(3, 1, &pSampler);
	CONTEXT->DSSetSamplers(3, 1, &pSampler);
	CONTEXT->HSSetSamplers(3, 1, &pSampler);
	CONTEXT->GSSetSamplers(3, 1, &pSampler);
	CONTEXT->CSSetSamplers(3, 1, &pSampler);
	...
	CreateFontFile("Default", TEXT("NotoSansKR-Bold.otf"));
	CreateFontFile("SkulFont", TEXT("a견고딕.ttf"));
	
	const TCHAR* FontFaceName = GetFontFaceName("Default");
	CreateTextFormat("Default", FontFaceName, 100, 20.f, TEXT("ko"));

	FontFaceName = GetFontFaceName("SkulFont");
	CreateTextFormat("SkulFont", FontFaceName, 600, 20.f, TEXT("ko"));
...
bool CResourceManager::LoadTextureFilePath(const std::string& Name, const TCHAR* FilePath, const std::string& PathName)
{
	CTexture* pTexture = FindTexture(Name);
	...
	if (!pTexture->LoadTextureFilePath(Name, FilePath, PathName))
	{
		SAFE_RELEASE(pTexture);
		return false;
	}
...
bool CResourceManager::LoadTextureFilePath(const std::string& Name, const std::vector<const TCHAR*>& vecFilePath)
{
	return true;
}
	...
	D3D11_TEXTURE_ADDRESS_MODE AddrV, D3D11_TEXTURE_ADDRESS_MODE AddrW)
{
	...
	/*
	3D 그래픽에서, 텍스처를 생성할 때 모델링의 각 꼭짓점에 대한 좌표를 2D 평면상의 전개도(TGA나 DDS와 같은 평면 이미지 파일)에 올리는 작업을 'UVW 언래핑'이라고 하며
	그로서 만들어지는 모델링 고유의 일종의'좌표 지도'를 'UVW 맵'이라고 하는데, 이 때 사용되는 용어.
	특별한 의미를 지닌 것은 아니고, 텍스처상에서 좌표를 대신하는 제 2의 좌표계 정도로 생각하는 것이 편하다.

	여담으로 UVW는 XYZ의 알파벳 내림차순 바로 앞 세 글자들이다. (U-V-W-X-Y-Z) 또한 쉽게 예상할 수 있는 부분이기도 하지만, 각각 U-X, V-Y, W-Z축을 의미한다.
	*/
	Desc.Filter = Filter;
	Desc.AddressU = AddrU; // 0에서 1 범위를 벗어난 au 텍스처 좌표를 해결하는 데 사용할 메서드를 지정.
	Desc.AddressV = AddrV; // 0에서 1 범위를 벗어난 av 텍스처 좌표를 해결하는 데 사용할 메서드를 지정.
	Desc.AddressW = AddrW; // 0에서 1 범위를 벗어난 aw 텍스처 좌표를 해결하는 데 사용할 메서드를 지정.
	Desc.MaxAnisotropy = MaxAnisotropy;	// 비등방성 최대치. (비등방성 : 방향에 따라 물체의 물리적 성질이 다른 것을 말한다.
										//					예를 들어, 솔질이 된 알루미늄, 섬유, 옷감, 근육 등의 표면은 들어오는 빛의 방향에 따라 반사율이 다른 광학적 비등방성을 띈다.)
	Desc.ComparisonFunc = D3D11_COMPARISON_NEVER; // 샘플링 된 데이터를 기존 샘플링된 데이터와 비교하는 함수. (D3D11_COMPARISON_NEVER : 비교하지 않겠다!)
	Desc.MinLOD = -FLT_MAX;
	Desc.MaxLOD = FLT_MAX;

	for (int i = 0; i < 4; ++i)
	{
		Desc.BorderColor[i] = 1.f; // D3D11_TEXTURE_ADDRESS_BORDER가 AddressU, AddressV 또는 AddressW에 대해 지정된 경우 사용할 테두리 색상.
	}
...
bool CResourceManager::AddAnimationSequence2DFrame(const std::string& SequenceName, unsigned int x, unsigned int y, unsigned int Count, const Vector2& FrameSize)
{
	CAnimationSequence2D* pSequence = FindAnimationSequence2D(SequenceName);
	...
	Vector2 Start = Vector2(FrameSize.x * x, FrameSize.y * y);

	for (int i = 0; i < Count; ++i)
	{
		pSequence->AddFrame(Start, Start + FrameSize);

		// 아틀라스의 왼쪽 끝에 도달했을 경우 Start Pos를 한 칸 아래 왼쪽으로 변경.
		if (Start.x + FrameSize.x >= pSequence->GetTexture()->GetTextureResourceInfo()->Width)
		{
			Start.y += FrameSize.y;
			Start.x = 0;
		}
