...
void CMaterial::SetStartEndUV(unsigned int x, unsigned int y, const Vector2& FrameSize)
{
	CTexture* Texture = m_vecTexture[0]->pTexture;
	Vector2 TextureSize(Texture->GetWidth(), Texture->GetHeight());

	Vector2 StartPos(x * FrameSize.x, y * FrameSize.y);
	Vector2 EndPos = StartPos + FrameSize;

	m_StartUV = StartPos / TextureSize;
	m_EndUV = EndPos / TextureSize;
}

void CMaterial::SetStartUV(const Vector2& UV)
{
	m_StartUV = UV;
}

void CMaterial::SetStartUV(float u, float v)
{
	m_StartUV = Vector2(u, v);
}

void CMaterial::SetEndUV(const Vector2& UV)
{
	m_EndUV = UV;
}

void CMaterial::SetEndUV(float u, float v)
{
	m_EndUV = Vector2(u, v);
}

void CMaterial::SetRenderState()
{
...
void CMaterial::SetTintColor(const Vector4& Color)
{
	m_Tint = Color;
}

void CMaterial::SetTintColor(float r, float g, float b, float a)
{
	m_Tint = Vector4(r, g, b, a);

	m_pCBuffer->SetTint(m_Tint);
}

void CMaterial::SetTintColor(unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
	m_Tint = Vector4(r, g, b, a);
...
void CMaterial::SetAddUV(const Vector2& UV)
{
	m_AddUV = UV;
}

void CMaterial::AddUV(const Vector2& UV)
{
	m_AddUV += UV;
}
...
void CMaterial::SetMaterial()
{
	...
	m_pCBuffer->SetAddUV(m_AddUV);
	m_pCBuffer->SetTint(m_Tint);
	m_pCBuffer->SetStartUV(m_StartUV);
	m_pCBuffer->SetEndUV(m_EndUV);
		...
		if (InfoCount == 1)
		{
			...
			if (Enable)
			{
				...
				if (Enable)
				{
					FilePathEnable = true;
					fread(&Length, sizeof(int), 1, pFile);
					fread(FilePath, sizeof(TCHAR), Length, pFile);
				}
			}
			...
			if (Enable)
			{
				...
				//const PathInfo* pInfo = CPathManager::GetInst()->FindPath(BASE_PATH);
				//char charBuff[] = "Bin\\";
				//TCHAR szUniCode[MAX_PATH] = {};
				//TCHAR szUniCode2[MAX_PATH] = {};
				//
				//MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, charBuff, strlen(charBuff), szUniCode, 260);
				//TCHAR* ResultPath;
				//ResultPath = wcsstr(TextureFullPath, szUniCode);
			...
			else if (FilePathEnable)
			{
				CResourceManager::GetInst()->LoadTextureFilePath(TextureName,
					FilePath, PathName);
			}
			...
			for (int i = 0; i < InfoCount; ++i)
			{
				...
				if (Enable)
				{
					...
					if (Enable)
					{
						...
						fread(&Length, sizeof(int), 1, pFile);
						fread(FilePath, sizeof(char), Length, pFile);
