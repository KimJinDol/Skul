...
bool CShaderManager::Init()
{
    ...
    if (!CreateShader<CBackgroundShader>("BackgroundShader"))
        return false;

    if (!CreateShader<CCoolTimeShader>("CoolTimeShader"))
        return false;

    // 상수버퍼를 만들어준다.
    CreateConstantBuffer("TransformCBuffer", sizeof(TransformCBuffer), 0, CBT_ALL);
    CreateConstantBuffer("MaterialCBuffer", sizeof(MaterialCBuffer), 1, CBT_ALL);
    ...
    CreateConstantBuffer("CoolTimeCBuffer", sizeof(CoolTimeCBuffer), 12, CBT_PIXEL);
