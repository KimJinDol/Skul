...
cbuffer GlobalCBuffer : register(b2)
{
	...
	float	g_CameraAreaRatio;
	float3	g_GlobalEmpty2;
	float	g_Contrast;
	float3	g_GlobalEmpth3;
};
...
SamplerState	g_BackgroundSmp	:register(s3);
...
float4 Distortion(float4 Color, float2 UV, float4 ProjPos)
{
	...
	*/
	...
	// 자기 자신을 일그러뜨리는 것이라면 뒷 배경의 픽셀값과 혼합되어야 한다.
...
float4 DistortionPostProcess(float4 Color, float2 UV, float4 ProjPos)
{
	...
	이 때 결과는 x, y, z, w 값이 나온다.

	다렉에서는 내부적으로 이 다음 단계를 실행한다.
	x / w, y / w, z / w, w / w를 해주어 w는 1이 되고 나머지 값들은 -1 ~ 1 사이의 값을 갖게 된다.
	또 -1 ~ 1 사이를 Screen 좌표로 변환해주게 된다.

	예를 들어 1280, 720 이라면
	-1 ~ 1 사이의 x값에 1280 / 2를 곱해주어 -640 ~ 640사이의 값이 되도록 한 후 다시 1280 / 2를 더한다.
	그러면 최종 0 ~ 1280 사이의 화면 좌표로 변환이 된다.
	*/
...
// -----------------------------------------------------------
// Null Buffer : 버텍스 버퍼, 인덱스 버퍼 등의 모든 정보가 Null인 버퍼. 인스턴싱 할 떄 인스턴스ID처럼 버텍스ID가 들어온다.
// 위의 특성 때문에 Null Buffer는 주로 화면에 꽉 차는 풀 스크린 사각형을 출력할 때 사용된다.
static const float2 g_NullPos[4] =
{
	float2(-1.f, 1.f),	// 왼쪽 상단
	float2(1.f, 1.f),	// 오른쪽 상단
	float2(-1.f, -1.f),	// 왼쪽 하단
	float2(1.f, -1.f)	// 오른쪽 하단
};
...
int ComputeCameraAreaRatio(float4 Pos)
{
	if (Pos.y < ((1.f - g_CameraAreaRatio) * (g_Resolution.y * 0.5f)))
		return 0;

	if (Pos.y > g_Resolution.y - ((1.f - g_CameraAreaRatio) * (g_Resolution.y * 0.5f)))
		return 0;
