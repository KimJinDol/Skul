...
private:
	...
	// 샘플러는 쉐이더 단계에서 샘플러 스테이지를 경유해 텍스처에 엑세스함으로써
	// 필터링 처리나 샘플링 처리를 수행한 값을 얻게 해 준다.
	// 샘플러를 경유하여 텍스처에 엑세스 할 땐 0~1로 정규화된 비율로써 값을 사용하기 때문에
	// 텍스처 크기를 변경할 때 픽셀 좌표에 대해 일일이 신경 쓰지 않아도 된다.
...
public:
	...
	bool LoadTexture(const std::string& Name,
		const std::vector<const TCHAR*>& vecFileName,
		const std::string& PathName = TEXTURE_PATH);
	bool LoadTextureFilePath(const std::string& Name, 
		const std::vector<const TCHAR*>& vecFilePath);
...
public:
	/*
	D3D11_FILTER_MIN_MAG_MIP_LINEAR : 축소, 확대 및 밉 레벨 샘플링에 선형 보간을 사용하겠다.
	*/
	/*
	enum D3D11_TEXTURE_ADDRESS_MODE
	{
		D3DX11_TEXTURE_ADDRESS_WRAP			: 텍스처 좌표를 반복한다.
		D3DX11_TEXTURE_ADDRESS_MIRROR		: 인접한 이미지를 반복한다.
		D3DX11_TEXTURE_ADDRESS_CLAMP		: 텍스처 끝 부분의 픽셀이 늘어진다.
		D3DX11_TEXTURE_ADDRESS_BORDER		: 경계색을 설정한다.
		D3DX11_TEXTURE_ADDRESS_MIRROR_ONCE	: CLMAP + MIRROR 모드
	}
	*/
...
public:
	...
	bool AddAnimationSequence2DFrame(const std::string& SequenceName, unsigned int x, unsigned int y,
		unsigned int Count, const Vector2& FrameSize = Vector2(70.f, 70.f));
